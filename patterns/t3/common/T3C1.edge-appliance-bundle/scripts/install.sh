...
usage() {
  printf '%s\n' "Install T3C1.edge-appliance-bundle artifacts for Debian/RPi OS."
...
run install -m 0644 "$pattern_dir/units/example.service" "$unit_dir/muster-appliance-health.service"
run install -m 0644 "$pattern_dir/units/muster-appliance.target" "$unit_dir/muster-appliance.target"
run install -m 0644 "$pattern_dir/units/muster-recover@.service" "$unit_dir/muster-recover@.service"
run install -m 0755 "$pattern_dir/scripts/appliance-doctor.sh" "$lib_dir/appliance-doctor.sh"
run install -m 0755 "$pattern_dir/scripts/muster-recover.sh" "$lib_dir/muster-recover.sh"
printf '%s\n' "install plan complete for T3C1.edge-appliance-bundle"
