...
usage() {
  printf '%s\n' "Install R3.cgroup-governor artifacts."
...
run install -m 0644 "$pattern_dir/units/example.service" "$unit_dir/muster-cgroup-governor.service"
run install -m 0755 "$pattern_dir/scripts/cgroup-governed-run.sh" "$lib_dir/cgroup-governed-run.sh"

printf '%s\n' "install plan complete for R3.cgroup-governor"
