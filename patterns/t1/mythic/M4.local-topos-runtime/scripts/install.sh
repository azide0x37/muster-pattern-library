...
usage() {
  printf '%s\n' "Install M4.local-topos-runtime artifacts."
...
run install -m 0644 "$pattern_dir/units/example.service" "$unit_dir/muster-local-topos-runtime.service"
run install -m 0755 "$pattern_dir/scripts/topos-evaluate.sh" "$lib_dir/topos-evaluate.sh"

printf '%s\n' "install plan complete for M4.local-topos-runtime"
