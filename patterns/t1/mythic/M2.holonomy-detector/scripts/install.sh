...
usage() {
  printf '%s\n' "Install M2.holonomy-detector artifacts."
...
run install -m 0644 "$pattern_dir/units/example.service" "$unit_dir/muster-holonomy-detector.service"
run install -m 0755 "$pattern_dir/scripts/holonomy-check.sh" "$lib_dir/holonomy-check.sh"

printf '%s\n' "install plan complete for M2.holonomy-detector"
