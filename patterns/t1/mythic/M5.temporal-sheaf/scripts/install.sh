...
usage() {
  printf '%s\n' "Install M5.temporal-sheaf artifacts."
...
run install -m 0644 "$pattern_dir/units/example.service" "$unit_dir/muster-temporal-sheaf.service"
run install -m 0755 "$pattern_dir/scripts/temporal-sheaf-run.sh" "$lib_dir/temporal-sheaf-run.sh"

printf '%s\n' "install plan complete for M5.temporal-sheaf"
