...
usage() {
  printf '%s\n' "Install M1.local-truth-sheaf artifacts."
...
run install -m 0644 "$pattern_dir/units/example.service" "$unit_dir/muster-local-truth-sheaf.service"
run install -m 0755 "$pattern_dir/scripts/local-truth-reduce.sh" "$lib_dir/local-truth-reduce.sh"

printf '%s\n' "install plan complete for M1.local-truth-sheaf"
