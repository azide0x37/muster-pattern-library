...
root=${MUSTER_ROOT:-}
apply=0
current_link="$root/opt/$project/current"
ledger_dir="$root/var/lib/muster/lifecycle/$project"
previous_file="$ledger_dir/previous-target"

usage() {
  printf '%s\n' "Rollback T2R5.signed-update-rail to the previous current target."
...
if [ ! -f "$previous_file" ]; then
  printf '%s\n' "no previous update target recorded: $previous_file" >&2
  exit 1
fi
...
printf '%s\n' "rollback plan complete for T2R5.signed-update-rail"
