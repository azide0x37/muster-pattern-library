...
for arg in "$@"; do
  case "$arg" in
    --strict) strict=1 ;;
    -h|--help) printf '%s\n' "Check T2R4.device-triggered-conveyor artifacts. Use --strict to require systemd-analyze."; exit 0 ;;
...
wait_script="$repo_root/patterns/t2/common/T2C1.hot-cold-nas-conveyor/scripts/wait-for-hot-capacity.sh"
...
printf '%s\n' "ok: T2R4.device-triggered-conveyor"
